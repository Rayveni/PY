...
class my_jira:
    ...
    def _get_json(self,_url:str):  
        r=self.session.get(_url,verify=self.verify_cert)
        return r.json()
        
    def issue_raw_info(self,issue_key:str):
    ...
    def find_issues(self,max_results:int=1000,**kwargs):
        ...
        custom:str
        """
        list_to_str=lambda l:','.join(l) if isinstance(l, list) else l
        and_operator=self.endpoints['and_operator']
        jql_parts=[self.endpoints['find_issues'][key].format(list_to_str(kwargs[key])) for key in kwargs.keys()]
        endpoint_url='search?jql={body}&maxResults={max_results}'.format(body=and_operator.join(jql_parts),max_results=max_results)
    ...
    def get_worklog(self,**kwargs):
        ...
        end_date:str
        """
        url=self._url(self.__endpoint_builder(self.endpoints['search_worklog'],kwargs))
        return self._get_json(url)
    ...
    def get_task_transitions(self,issue_key:str):
        url=self._url(self.endpoints['transition_issue'].format(issue_key=issue_key))           
        return self._get_json(url)
    ...
    def issue_to_sprint(self,sprint_id:int,issue_keys:list):
        url=self._url(self.endpoints['sprint_issue'].format(sprint_id=sprint_id),api_version=self.agile_api_version)
        issue_data={'issues':issue_keys}
        return self.session.post(url,data=dumps(issue_data),verify=self.verify_cert)
    def issue_to_epic(self,epic_id:int,issue_keys:list):
        url=self._url(self.endpoints['epic_issue'].format(epic_id=epic_id),api_version=self.agile_api_version)
        issue_data={'issues':issue_keys}
        return self.session.post(url,data=dumps(issue_data),verify=self.verify_cert)
...
class my_jira_reports(my_jira):
    ...
    def extract_issue_info(self,issue):
        conv_to_date =lambda s:None if s is None else datetime.strptime(s,'%Y-%m-%d').date()
        res={'key':issue['key']}
        fields=issue['fields']
        res['assignee']=self.__try_get_el_from_dict( fields,['assignee','displayName']) 
        res['parent']=self.__try_get_el_from_dict( fields,['parent','key']) 
        res['resolution']=self.__try_get_el_from_dict( fields,['resolution','name']) 
        ...
        res['timeoriginalestimate']=self.sec_to_day_weeks(fields['timeoriginalestimate'] )
        res['start_date']=conv_to_date(self.__try_get_el_from_dict( fields,['customfield_11164']))
        res['end_date']=conv_to_date(self.__try_get_el_from_dict( fields,['customfield_11163']))
        try:
            sprint=dict(map(lambda s:s.split("="),fields['customfield_10100'][0].split(',')))
    ...
    def undone_custom_report(self,project:str):
        def update_status(status:str,start_date,end_date):
            ...
            if end_date<=_now:
                ...
                res="ok"          
            return res 
        undone_sprint_issues=self.find_issues(project_list=[project],custom='statusCategory!=Done%20',max_results=1000)
        df=DataFrame(map(self.extract_issue_info,undone_sprint_issues))      
        df['term']=df['timeestimate'] +'/'+df['timeoriginalestimate']
        df['update_state']=df[['status','start_date','end_date']].apply(lambda row: update_status(*row), axis=1)

        df.sort_values(by=['sprint_state', 'end_date'],ascending=(True,False),inplace=True)
    ...
    def color_undone_custom_report(self,color_df): 
        ...
        style1=c_df.style.applymap(highlight_cols,subset=['update_state','sprint_state'])
        style2=c_df.style.applymap(color_term,subset=['term'])
        style2.use(style1.export())
        return style2
        
    def mass_transition(self,df)->list:
        _arr=df[(df.sprint_name.notnull()) & (df.update_state!="ok")][['key','update_state','status']].values.tolist()
